...
- 실수 모음
    - 설계 실수 :
        우선순위에 따라 이동할 때!! 단순 조건 분기로 하려다가 재설계
        단순 거리 계산인데 bfs로 설계하다가 구현난이도 높임
    - 인덱스 처리
        회전할 때 인덱스 처리 미흡해서 고생 ,,
    -
"""
"""
=============== 2차 코드 리뷰 =================
1628 문제 읽기 + 주석+ 설계
    설계 꽤나 자세하게 했다. 
    특히 미로 사각형 잡는거 인덱스 상세히 잡음
    
1651 구현시작
    - 좌표 회전 중간에 미흡해서 재설계
    - 사람 관리하는 방식 굳이 3차원 배열로 들고다닐 필요 없겠다 싶어서 
        그냥 lst 하나로만 관리
        이동했다가 특정 사람 위치를 보고 방향을 틀거나 하는 등의 로직이 전혀 없었기 때문
        
1511 디버깅 시작
    printa 함수 활용
    sum break문 위치 수정 필요 알아차림
    for문으로 탐색하는게 틀렸다는 것 파악
    전체 프린트 디버깅으로 체크하고 제출 ! 원트 정답 오예~
"""
